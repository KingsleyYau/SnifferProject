...
int DrUdpSocket::Bind(unsigned int iPort, string ip, bool bBlocking) {
	...
	if((m_Socket = socket(AF_INET, SOCK_DGRAM, 0/*IPPROTO_UDP*/)) < 0){
		// create socket error;
		ELog("Jni.DrUDPSocket.Bind", "create socket error");
	    return false;
	}
	...
	if(iRet >= 0) {
		DLog("Jni.DrUDPSocket.Bind", "当前接收缓冲区大小:%d", iBuffer);
	}

	iRet = getsockopt(m_Socket, SOL_SOCKET, SO_SNDBUF, &iBuffer, NULL);
	if(iRet >= 0) {
		DLog("Jni.DrUDPSocket.Bind", "当前发送缓冲区大小:%d", iBuffer);
	}
	...
	if(m_bBlocking) {
		...
		if ((iFlag = fcntl(m_Socket, F_GETFL, 0)) == -1) {
			ELog("Jni.DrUDPSocket.Bind", "fcntl F_GETFL O_NONBLOCK socket error");
			return iRet;
		}
		if (fcntl(m_Socket, F_SETFL, iFlag & ~O_NONBLOCK) == -1) {
			ELog("Jni.DrUDPSocket.Bind", "fcntl F_SETFL iFlag & ~O_NONBLOCK socket error");
			return iRet;
		}
		DLog("Jni.DrUDPSocket.Bind", "set socket to blocking");
	}
	else {
		// set to nonblocking
		int iFlag = 1;
		if ((iFlag = fcntl(m_Socket, F_GETFL, 0)) == -1) {
			ELog("Jni.DrUDPSocket.RecvData", "fcntl F_GETFL O_NONBLOCK socket error");
			return iRet;
		}
		if (fcntl(m_Socket, F_SETFL, iFlag | O_NONBLOCK) == -1) {
			ELog("Jni.DrUDPSocket.RecvData", "fcntl F_SETFL iFlag | O_NONBLOCK socket error");
			return iRet;
		}
		DLog("Jni.DrUDPSocket.Bind", "set socket to nonblocking");
	}
	...
	if(bind(m_Socket, (struct sockaddr *)&localAddr, sizeof(localAddr)) < 0) {
		// bind socket error
		ELog("Jni.DrUDPSocket.Bind", "bind socket error");
		return false;
	}
...
int DrUdpSocket::SendData(string ip, unsigned int iPort, char* pBuffer, unsigned int uiSendLen) {
	...
	DLog("jni.DrUdpSocket::SendData", "准备向(%s:%d)发送数据，Hex编码后:\n%s(%d)", ip.c_str(), iPort, log.c_str(), uiSendLen);
	...
	if(m_Socket > 0) {
		...
		if((sendSocket = socket(AF_INET, SOCK_DGRAM, 0/*IPPROTO_UDP*/)) < 0){
			// create socket error;
			DLog("Jni.DrUDPSocket.SendData", "create socket error");
		    return iRet;
		}
	}
...
int DrUdpSocket::RecvData(char* pBuffer, unsigned int uiRecvLen, unsigned int uiTimeout, struct sockaddr_in* remoteAddr) {
	...
	socklen_t iSendAddrLen = sizeof(struct sockaddr_in);
	...
	if(m_bBlocking) {
		...
		iRet = recvfrom(m_Socket, pBuffer, uiRecvLen, 0, (struct sockaddr *)&sendAddr , (socklen_t *) &iSendAddrLen);
		...
		DLog("jni.DrUdpSocket::RecvData", "收到来自(%s:%d)访问，成功接收字节数:(%d) Hex编码后:\n%s", ip.c_str(), iPort, iRet, log.c_str());
	}
	else {
		// nonblocking recv
		timeval tout;
		...
		if(iRetS > 0) {
			...
			DLog("jni.DrUdpSocket::RecvData()", "%d秒超时返回！", uiTimeout / 1000);
		}
		else {
			DLog("jni.DrUdpSocket::RecvData()", "socket错误返回！");
		}
	}
