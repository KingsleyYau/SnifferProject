	...
	if(m_bBlocking) {
		...
		while (true) {
			...
			if (iRetS == -1) {
                ...
                iRet = recv(m_Socket, pBuffer, uiRecvLen, 0);
                if(iRet == 0) {
                    // 对方close socket，发来bye bye
                    DLog("Jni.DrTcpSocket::RecvData()", "客户端主动断开连接!");
                    iRet = iRecvedLen;
                    bAlive = false;
                    goto EXIT_TCP_RECV;
                }
